...
| [CJM01 — 스포츠 OTT 접근성](./CJM/CJM01-스포츠OTT-접근성.md) | 페르소나 12인 전원의 개별 여정 지도, 단계별 Pain Point와 개선 기회 |

> **결론 요약** — 열두 사람의 여정에서 공통 Pain Point 다섯 개가 나왔고, 그중 **넷이 기능을 더 만드는 문제가 아니었습니다.** 배치(접근성 메뉴에만 있음), 지연(정확도보다 지연이 만족을 가름), 표현(안내가 과거 실망과 이어짐), 계측(효과가 데이터로 남지 않음)의 문제입니다. 다섯 번째는 감각 채널·지불 능력·기기가 만드는 **구조적 배제**로, 제품 개선으로는 풀리지 않는 경계선입니다. 열두 사람의 단계 구성은 전부 달랐습니다 — 양식 하나를 열두 번 채운 것이 아니라 열두 번 새로 정의했고, 그 이유를 방법론 맨 끝에 경고로 남겼습니다.
